SELECT
  ...
  JSON_EXTRACT(json(data), '$.EventData.Application Path') AS 'application-path',
  JSON_EXTRACT(json(data), '$.EventData.url') AS 'url',
  JSON_EXTRACT(json(data), '$.EventData.RuleGUID') AS 'rule-GUID',
  JSON_EXTRACT(json(data), '$.EventData.RuleName') AS 'rule-name',
  JSON_EXTRACT(json(data), '$.EventData.RuleSetGUID') AS 'configuration-Guid',
  eventid
FROM
  windows_eventlog
WHERE
